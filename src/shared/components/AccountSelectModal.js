...
export default function AccountSelectModal({
  ...
  return (
    <Modal
      ...
      size="md"
      footer={null}
      bodyClassName="flex flex-col gap-3 p-4 max-h-[calc(85vh-64px)] overflow-hidden"
    >
      {/* Info bar */}
      <div className="shrink-0 flex items-start gap-2 px-2.5 py-2 bg-primary/8 border border-primary/20 rounded-lg text-xs text-text-muted leading-relaxed">
        <span className="material-symbols-outlined text-primary shrink-0 mt-px" style={{ fontSize: "14px" }}>info</span>
        <span>
          Tap to add or remove — saved automatically.
          {" "}
          {selectedIds.length > 0
            ? <span className="text-text-main font-medium whitespace-nowrap">{selectedIds.length} selected.</span>
            : <span className="text-text-main font-medium">None selected: every account is allowed.</span>}
        </span>
      </div>
      ...
      <div className="shrink-0 relative">
        ...
        <input
          ...
          className="w-full pl-8 pr-3 py-2 bg-surface border border-border rounded-lg text-xs focus:outline-none focus:ring-1 focus:ring-primary/50"
        />
      </div>

      {/* Accounts grouped by provider — the only scrolling region in this modal */}
              ...
              {group.connections.map((conn) => {
                ...
                return (
                  <button
                    ...
                    className={`
                      px-2.5 py-1.5 rounded-xl text-xs font-medium transition-all border hover:cursor-pointer
                      ${isSelected
