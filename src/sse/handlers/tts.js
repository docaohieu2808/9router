...
export async function handleTts(request) {
  ...
  if (comboModels) {
    ...
    return handleComboChat({
      body,
      models: comboModels,
      handleSingleModel: (b, m) => handleSingleModelTts(b, m, responseFormat, language, style, allowedConnectionIds),
      log,
      comboName: modelStr,
      comboStrategy,
      comboStickyLimit,
  ...
  return handleSingleModelTts(body, modelStr, responseFormat, language, style, allowedConnectionIds);
}

async function handleSingleModelTts(body, modelStr, responseFormat, language, style, allowedConnectionIds = null) {
  ...
  if (!CREDENTIALED_PROVIDERS.has(provider)) {
    const result = await handleTtsCore({ provider, model, input: body.input, responseFormat, language, style, refAudio: body.reference_audio, refText: body.reference_text });
  ...
  while (true) {
    ...
    const result = await handleTtsCore({ provider, model, input: body.input, credentials, responseFormat, language, style, refAudio: body.reference_audio, refText: body.reference_text });
