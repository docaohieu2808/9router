...
import { getAllowedConnectionIdsForKey } from "@/lib/localDb";
...
export async function buildModelsList(kindFilter, options = {}) {
  ...
  try {
    ...
    // Per-key account scoping: a key restricted to a subset of connections must
    // not be shown models it will only be refused on. null = unrestricted key.
    if (options.allowedConnectionIds) {
      connections = connections.filter(c => options.allowedConnectionIds.has(c.id));
    }
  } catch (e) {
...
export async function GET(request) {
  try {
    ...
    const data = await buildModelsList([LLM_KIND], { skipDynamicFetch, allowedConnectionIds });
