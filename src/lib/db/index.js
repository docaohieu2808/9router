...
export {
  ...
  getAllowedConnectionIdsForKey, getKeyAccounts, setKeyAccounts,
...
export async function exportDb() {
  ...
  const out = {
    ...
    apiKeyAccounts: db.all(`SELECT * FROM apiKeyAccounts`).map((r) => ({ apiKeyId: r.apiKeyId, connectionId: r.connectionId })),
...
export async function importDb(payload) {
  ...
  db.transaction(() => {
    ...
    // Restore key→account scoping after apiKeys and providerConnections exist so
    // both foreign keys resolve; OR IGNORE drops rows whose targets are absent.
    for (const m of payload.apiKeyAccounts || []) {
      db.run(`INSERT OR IGNORE INTO apiKeyAccounts(apiKeyId, connectionId) VALUES(?, ?)`, [m.apiKeyId, m.connectionId]);
    }
