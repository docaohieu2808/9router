 ...
 */
export async function handleTtsCore({ provider, model, input, credentials, responseFormat = "mp3", language, style, refAudio, refText }) {
  if (!input?.trim()) {
  ...
  try {
    ...
    if (adapter) {
      // refAudio/refText are only meaningful to voice-cloning adapters (omnivoice); every
      // other adapter ignores the extra options.
      const result = await adapter.synthesize(input.trim(), model, credentials, responseFormat, { language, style, refAudio, refText });
